 ...
 */
     ...
     * lookDirection hjälper till att hålla reda på vart du tittar i förhållande till
    ...
    private void printWelcome()
    {
        ...
        System.out.println(rooms.get(location).getDescription(this));
    }
    ...
    private boolean processCommand(Command command) 
    {
        ...
        if(commandWord.equals("quit") || commandWord.equals("exit")) return true;
        //Commandot var tydligen inte "quit" eller "exit".
    ...
    private void goRoom(Command command) 
    {
        ...
        int direction = 5;
        if(directionString.equals("left")) direction = (lookDirection + 3) % 4;
        if(directionString.equals("forward")) direction = lookDirection;
        if(directionString.equals("right")) direction = (lookDirection + 1) % 4;
        if(directionString.equals("backwards") ||directionString.equals("backward") || directionString.equals("back"))
            direction = (lookDirection + 2) % 4;
        ...
        if(rooms.get(location).getDoor(direction) != null){
            if(rooms.get(location).getDoor(direction).getStatus()){
                ...
                if(Math.random() <= 0.05){
                     ...
                     * (5 - lookDirection) % 4 bestämmer hur mycket riktningen måste modifieras för att överensstämma med de interna riktningarna.
                     ...
                     */
                    rooms.get(location).getDoor(((5-lookDirection)% 4 + 3)%4).setStatus(true);
                }
                return;
            }
        }
     ...
     * A non existing room will generate a new room.
     */
    public void teleport(Point where){
        location = where;
        while(!rooms.containsKey(location)){
                //skapar ett nytt rum, låser upp alla dörrar den stöter på. Ifall rummet saknar dörr skapas ett nytt rum.
                Room newRoom = new Room(new Point(location), rooms);
                for(int i = 0; i < 4; i++){
                    Door door = newRoom.getDoor(i);
                    if(door != null){
    ...
    /**
     * Translates the user input direction to an internal direction the game can handle.
     */
    
    public int translateToInternalDirection(int externalDirection){
        return (((5 - lookDirection) % 4) + externalDirection) % 4;
    }
}
