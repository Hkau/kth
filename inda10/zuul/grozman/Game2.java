...
public class Game 
{
    ...
    private Room currentRoom, beamTo, winRoom;
    ...
    {
        Room outside, theatre, heaven, lab, office;
      
        // create the rooms
        outside = new Room("outside the main entrance of the university");
        theatre = new Room("in a lecture theatre");
        heaven = new Room("in the Garden of Eden");
        ...
        outside.setExit("west", heaven, true);

        //theatre.setExit("west", outside, false);

        heaven.setExit("east", outside, false);
        ...
        if (commandWord.equals("help")) {
            ...
            if(tryToGo(command)) {
                ...
                if(currentRoom == winRoom) {
                    System.out.println("Epic win, congrats.");
                    return true;
                }
                else if(currentTime >= TIME_LIMIT) {
    ...
    private boolean tryToGo(Command command) {
        ...
        if(currentRoom.isExitLocked(command.getSecondWord())) {
            ...
            if(!hasKey)  {
                System.out.println("To obtain THE KEY, you need to solve");
                ...
                if(c == a + b) {
                    System.out.println("You have solved the difficult differential equation and obtained THE KEY.");
                    hasKey = true;
                }
                else {
                    System.out.println("You are not worthy of THE KEY.");
                    return false;
                }
            }
            System.out.println("Would you like to unlock THE DOOR and enter at this time? (y/n)");
            if(!parser.getCommand().getSecondWord().toLowerCase().startsWith("y")) return false;
        }
