...
int main()
{
	...
	//sudoku_casting();
	srand( time(NULL) );
	sudoku_state *s1 = simple_heuristic();
	sudoku_state *s2 = simple_heuristic();

    if(s1->num_actors+s1->num_super < s2->num_actors+s2->num_super)
        print_output(s1);
    else
        print_output(s2);
...
sudoku_state *first_state()
{
	...
	for(i = 1; i <= prod.num_roles; ++i)
	{
		if(prod.roles[i].num_actors == 1)
			s->assign_role(i, prod.roles[i].actors[0]);
